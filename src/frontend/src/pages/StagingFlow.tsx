...
import { aimlTxt2Img } from "../hooks/useAimlApi";
...
export default function StagingFlow({
  ...
  const generateStagedImage = async (orderId: string) => {
    ...
    // Heartbeat: keeps progress creeping from 90% to 98% while AI is running
    ...
    try {
      // Animate progress steps up to 90%
      ...
      // Start heartbeat animation while AI is processing
      setProgressLabel("AI is generating your staged room...");
      startHeartbeat();

      let imageUrl: string | null = null;
      try {
        // Race the AI call against a 120-second timeout
        const generatedUrl = await Promise.race([
          aimlTxt2Img(prompt, imageBase64 ? { imageBase64 } : undefined),
        ...
        if (errMsg === "timeout") throw aiErr;
        // Fall back to placeholder on non-timeout errors
        console.warn("AIML API error (using fallback):", aiErr);
        imageUrl =
