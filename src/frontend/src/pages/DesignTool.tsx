...
import { aimlTxt2Img, aimlTxt2Vid } from "../hooks/useAimlApi";
...
export default function DesignTool({
  ...
  const handleVideoSend = async () => {
    ...
    startProgress(true);
    try {
      const videoUrl = await aimlTxt2Vid(videoPrompt, {
  ...
  // Convert any image URL (CDN, blob, data) to a base64 data URL
  const toDataUrl = async (url: string): Promise<string> => {
    ...
    try {
      ...
      // If cross-origin fetch fails, return url as-is
      return url;
    }
  };
  ...
  const handleSend = async () => {
    ...
    if (refineTargetId) {
      ...
      try {
        ...
        // Use last generated image as base if available, otherwise use original upload
        const rawBaseImageUrl =
          generations.length > 0
            ? generations[generations.length - 1].generatedImageUrl
            : uploadedImageUrl;
        const baseImageUrl = await toDataUrl(rawBaseImageUrl);

        const inputBase64 = baseImageUrl.startsWith("data:")
          ? baseImageUrl.split(",")[1]
          : baseImageUrl;
        const inputMime = baseImageUrl.startsWith("data:")
          ? baseImageUrl.split(";")[0].replace("data:", "")
          : "image/jpeg";
        const generatedUrl = await aimlTxt2Img(prompt, {
        ...
        // Log AI generation (fire-and-forget)
        if (actor)
          actor
            .logAiGeneration(prompt, baseImageUrl, newGen.generatedImageUrl)
            .catch(() => {});
        await recordPhotoUsageLocal();
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto">
            {chatItems.length === 0 && !isGenerating ? (
              ...
              >
                {uploadedImageUrl ? (
                  /* Image preview when a photo has been uploaded */
                  <motion.div
                    key="image-preview"
                  ...
                  >
                    ...
                    <div className="text-center">
                      <p
                        ...
                        style={{ color: "#111827" }}
                      >
                        {activeTool.label} — ready to transform
                      </p>
                      <p className="text-sm" style={{ color: "#6B7280" }}>
                        {isVideoMode
                          ? "Describe the animation below and hit Generate"
                  ...
                  /* Default icon placeholder when no image uploaded yet */
                  <>
                    <div
